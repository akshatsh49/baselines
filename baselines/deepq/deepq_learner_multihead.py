...
class DEEPQ_multihead(tf.Module):

    def __init__(self, q_func, observation_shape, num_actions, num_heads, batch_sampling_probability, lr, grad_norm_clipping=None, gamma=1.0,
        double_q=True, param_noise=False, param_noise_filter_func=None):

      self.num_actions = num_actions
      self.num_heads = num_heads
      self.batch_sampling_probability = batch_sampling_probability
      self.gamma = gamma
      ...
      with tf.name_scope('target_q_network'):
        self.target_q_network = q_func(observation_shape, num_actions, num_heads)
    ...
    def train(self, obs0, actions, rewards, obs1, dones, importance_weights):
      with tf.GradientTape() as tape:
        ...
        selected_action = tf.concat([tf.reshape(tf.one_hot(actions, self.num_actions, dtype=tf.float32), [-1,actions.shape[0],self.num_actions])] * self.num_heads, 0)
        q_t_selected = tf.reduce_sum(q_t * selected_action, 2)
        ...
        if self.double_q:
            ...
            q_tp1_best = tf.reduce_sum(q_tp1 * tf.one_hot(q_tp1_best_using_online_net, self.num_actions, dtype=tf.float32), 2)
        else:
        ...
        sample_batches = np.random.binomial(n = 1, size = self.num_heads * actions.shape[0], p = self.batch_sampling_probability).reshape(self.num_heads, actions.shape[0]) # train heads on different batches to disentangle q functions
        sample_batches = tf.constant(sample_batches, dtype=tf.float32)

        errors = errors*sample_batches
        weighted_error = tf.reduce_mean(importance_weights * errors)

    #   init_vars = [p.numpy() for p in self.q_network.trainable_variables]
    ...
    #   init_vars = [p.numpy() for p in target_q_vars]
